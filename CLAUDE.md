...
- **Don't misread an `Edit` failure as "unprotected".** `Edit` validates its `old_string` *before* the `PreToolUse` hook fires, so an edit to a red-listed file whose `old_string` doesn't match returns `String to replace not found` rather than the guard's block message. The file is still protected — the guard gates every edit that would actually write — but the error names the wrong reason, and reading it as "the red list didn't stop me" and retrying with a matching `old_string` walks straight into the block.
